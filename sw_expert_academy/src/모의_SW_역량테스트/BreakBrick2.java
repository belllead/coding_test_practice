...
//			-----------------
	...
	static void breakBreakRoutine(int[][] brickMap, int shotNum) {
		...
		for (int i=0; i<W; i++) {
			...
			for (int r=0; r<H; r++) {
				for (int c=0; c<W; c++) {
					System.out.print(columnTest[i][r][c]);
				}
				System.out.println();
			}
			System.out.println();
	...
	static int[][][] copyBrickMap(int[][] brickMap) {
		...
		for (int i=0; i<W; i++) {
			...
			for (int j=0; j<H; j++) {
				tempArr[j][W] = Arrays.copyOf(brickMap[j][W], W);
			}
//			copiedBrickMap[i] = ;
		}
